...
# -----------------------------
# Add "Normal" class with its own probability; reduce others proportionally.
# Original probabilities:
# Depression 0.25, Anxiety 0.20, Stress 0.20, Bipolar_Mania 0.15, Phobia 0.10, Suicidal_Tendency 0.10.
# Now include Normal with 0.10, and scale down the rest to sum to 1.
scale_factor = 0.9  # to accommodate 10% Normal
profiles = {
    "Depression":        0.25 * scale_factor,
    "Anxiety":           0.20 * scale_factor,
    "Stress":            0.20 * scale_factor,
    "Bipolar_Mania":     0.15 * scale_factor,
    "Phobia":            0.10 * scale_factor,
    "Suicidal_Tendency": 0.10 * scale_factor,
    "Normal":            0.10,
}
...
normal_mask   = profile_col == "Normal"
...
# Normal: set features to healthy levels
positive   = np.where(normal_mask, positive * 1.0, positive)   # keep around 0.05
negative   = np.where(normal_mask, negative * 0.5, negative)   # lower negative
fear       = np.where(normal_mask, fear    * 0.5, fear)
sadness    = np.where(normal_mask, sadness * 0.5, sadness)
anger      = np.where(normal_mask, anger   * 0.5, anger)
uncertainty= np.where(normal_mask, uncertainty * 0.6, uncertainty)
filler     = np.where(normal_mask, filler  * 0.6, filler)
rumination = np.where(normal_mask, rumination * 0.5, rumination)
self_ref   = np.where(normal_mask, self_ref * 0.5, self_ref)
coherence  = np.where(normal_mask, coherence * 1.1, coherence)  # high coherence
sentiment  = np.tanh((positive - negative) * 5)                 # will be near zero

# Clip all to natural bounds
...
sentiment = np.where(suicidal_mask, sentiment - 0.5, sentiment)  # extra negative
...
# Normal: slightly higher present focus, balanced
future = np.where(normal_mask,   future * 1.5,   future)
...
# Normal: neutral topic distribution (already uniform)
...
pbar.update(1)
df.to_csv("mental_health_synthetic_dataset_with_normal.csv", index=False)
pbar.close()
