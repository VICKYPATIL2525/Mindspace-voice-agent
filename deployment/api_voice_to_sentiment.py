...
async def lifespan(app: FastAPI):
    ...
    demo_sample_path = Path(__file__).parent.parent / "demo-api-input-data-sample" / "voice_normal_sample_1.json"
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("api_voice_to_sentiment:app", host="0.0.0.0", port=9001, reload=True)
# To run the API (from the project root directory):
# uvicorn deployment.api_voice_to_sentiment:app --reload --port 9100
# Then open http://localhost:9100/docs in the browser to access the interactive API docs (Swagger UI).