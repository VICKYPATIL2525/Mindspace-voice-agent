...
# ─── API Key auth ─────────────────────────────────────────────────────────────
# Read at request time via a function so changes to the env are picked up.
...
def verify_api_key(key: str = Security(_api_key_header)) -> None:
    ...
    if not api_key:
        raise HTTPException(status_code=500, detail="Server misconfiguration: MINDSPACE_TEXT_API_KEY not set.")
    if key != api_key:
...
class PredictRequest(BaseModel):
    ...
    # ── Fix #3: overall_sentiment_score has range [-1, 1], not [0, 1].
    # Validate it separately from the non-negative ratios.
    ...
    # ── Fix #4: perplexity is always >= 1 mathematically. Allow >= 1 check.
    ...
    # ── Fix #2: cross-field check — both language flags cannot be 1 at once.
...
def model_info(_: None = Security(verify_api_key)):
    ...
    # Return only non-sensitive structural info — no accuracy metrics exposed.
    return {
